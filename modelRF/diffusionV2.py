...
class GVPTransCond(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.num_layers = 4
        self.out_dim = 640 #LATENT SPACE
        self.time_cond = True
        self.dihedral_angle = True
        self.num_trans_layer = 8
        self.drop_struct = -1

        self.rna_encoder, self.alphabet = fm.pretrained.rna_fm_t12()
        self.alphabet_converter = self.alphabet.get_batch_converter()
        ...
        # Transformer Layers
        self.seq_res = nn.Linear(1280, self.node_h_dim[
            0])  #### (RNA seq, cond) expend dimension for cross attention with protein node features
        ...
        self.out_layer_norm = nn.LayerNorm(self.out_dim)
    ...
    def forward(self, batch, cond_drop_prob=0., **kwargs):
        ...
        z_t = batch.z_t  # latent vector for RNA_RM embedding
        ...
        # print('z_t', z_t.shape)
        # print('cond_x', cond_x.shape)
        
        init_seq = torch.cat([z_t, cond_x], -1)  # (len_RNA) x 8
        ...
        # print(init_seq.reshape(batch_size, length, -1).shape)
        # print(batch_size)
        RNA_base_seq = self.seq_res(init_seq.reshape(batch_size, length, -1))  # (batch_size, lenght, hidden_dim)
        ...
        logits = self.out_layer_norm(logits)
